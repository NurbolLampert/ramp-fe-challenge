# Changes by Nurbol Lampert

#### Bug 1: 
change position from fixed to absolute in ../src/index.css(.RampInputSelect--dropdown-container)
#### Bug 2: 
added input change for the toggle button in ../src/index.css(.RampInputCheckbox--input {
                                position: absolute;
                                opacity: 0;
                                z-index: 2;
                                width: 1.25rem; 
...
#### Bug 3: 
In the ../src/utils/requests.ts(getTransactionsByEmployee), when selected All Employees, the employeeId is null. So, instead of returning Error, we will return all data.transactions (which is transaction of All Employees)
#### Bug 4: 
In the ../src/hooks/usePaginatedTransactions.ts(fetchAll) is a recursive function which only returned the current data, which actually becomes the next one. The previousResponse is added so the previous transactions are recursively kept as the current ones for the next call.
#### Bug 5: 
In the ../src/App.tsx The InputSelect component's isLoading prop was incorrectly tied to isLoading state, causing the dropdown to display "Loading employees..." even when the employees had already loaded. Removed the isLoading state and its associated setter setIsLoading from App.tsx. Updated the InputSelect component's isLoading prop to use the employeeUtils.loading state directly. This ensures the "Loading employees..." state in the dropdown directly reflects the loading state of the useEmployees hook. (solved the issues for part 1 and 2)
#### Bug 6:
In the ../src/App.tsx utilizes the nextPage property from the paginatedTransactions state to determine if there's another page of data available. If nextPage is null, it means there's no more data to fetch, so the "View more" button should be hidden. Replaces: transactions !== null to => transactions !== null && transactionsByEmployee === null && paginatedTransactions?.nextPage !== null. (solved the issues for part 1 and 2)
#### Bug 7:
##### Creation of TransactionContext.tsx in Utils:
 - A new context file named TransactionContext.tsx was created within the src/utils directory.
 - This context provides centralized state management for transactions and offers methods to update the transaction data.
 - It uses mock-data.json to simulate transaction data.
##### Updating the TransactionPane Component:
 - In the TransactionPane.tsx component, the displayed transaction is derived directly from props.
 - To ensure the displayed transaction reflects any updates, the component should prioritize using the transaction data from the centralized context over the prop data.
 - The component can achieve this by searching the centralized transactions for a transaction with a matching ID and using that for rendering. If the transaction doesn't exist in the centralized context, the component can fall back to using the prop data.
